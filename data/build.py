...
from data.gta5.gta_dataloader import GTASCN
from data.semantic_kitti.semantic_kitti_dataloader import SemanticKITTISCN
...
def build_dataloader(cfg, mode='train', domain='source', start_iteration=0, halve_batch_size=False):
    ...
    if dataset_cfg.TYPE == 'SemanticKITTISCN':
        dataset = SemanticKITTISCN(split=split,
                                   **dataset_kwargs,
                                   **augmentation)
    elif dataset_cfg.TYPE == 'GTASCN':
        dataset = GTASCN(split=split,
                                   **dataset_kwargs,
                                   **augmentation)
    else:
